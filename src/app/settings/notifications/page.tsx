...
import { NotificationToggleForm } from "@/features/settings/components/notification-toggle-form";
...
export default async function NotificationSettingsPage() {
  ...
  const prefs = settings?.notification_preferences ?? { email: true, push: true };
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white p-4 md:p-6 pb-28">
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-5">
        <NotificationToggleForm emailEnabled={prefs.email} pushEnabled={prefs.push} />
      </div>
    </div>
  );
}
